...
export async function GET(req: NextRequest) {
  ...
  const inventoryTypes = await prisma.forestInventoryTypeCatalog.findMany({
    where,
    include: {
      organization: {
  ...
  const rows = inventoryTypes.map((item) => ({
    id: item.id,
    code: item.code,
    name: item.name,
  ...
  const headers = ["id", "code", "name", "isActive", "createdAt", "updatedAt", "organizationId", "organizationName"];
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="inventory_types_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheet = XLSX.utils.json_to_sheet(worksheetRows, {
    header: ["ID", "Código", "Nombre", "Activo", "Creado en", "Actualizado en", "Organización ID", "Organización"],
  });

  XLSX.utils.book_append_sheet(workbook, worksheet, "InventoryTypes");
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="inventory_types_${filenameDate}.xlsx"`,
    },
  });
}
