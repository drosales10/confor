...
import { inventoryTypeCreateSchema } from "@/validations/forest-config.schema";
...
export async function POST(req: NextRequest) {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const parsed = inventoryTypeCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.forestInventoryTypeCatalog.findFirst({
            where: {
              id: row.id,
      ...
      if (existing) {
        await prisma.forestInventoryTypeCatalog.update({
      ...
      await prisma.forestInventoryTypeCatalog.create({
        data: {
          organizationId: targetOrganizationId,
          code: parsed.data.code,
          name: parsed.data.name,
