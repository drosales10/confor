...
import { speciesCreateSchema } from "@/validations/forest-config.schema";
...
export async function POST(req: NextRequest) {
  ...
  if (ext === "csv") {
    ...
    const required = ["code", "scientificname"];
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const parsed = speciesCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.species.findFirst({
            where: {
              id: row.id,
