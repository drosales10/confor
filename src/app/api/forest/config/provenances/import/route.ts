...
import { provenanceCreateSchema } from "@/validations/forest-config.schema";
...
export async function POST(req: NextRequest) {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const parsed = provenanceCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.provenance.findFirst({
            where: {
              id: row.id,
