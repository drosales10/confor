...
import { managementSchemeCreateSchema } from "@/validations/forest-config.schema";
...
export async function POST(req: NextRequest) {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const parsed = managementSchemeCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.managementScheme.findFirst({
            where: {
              id: row.id,
      ...
      if (existing) {
        await prisma.managementScheme.update({
      ...
      await prisma.managementScheme.create({
        data: {
          organizationId: targetOrganizationId,
          code: parsed.data.code,
          name: parsed.data.name,
