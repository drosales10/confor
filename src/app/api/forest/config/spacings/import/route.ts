...
import { spacingCreateSchema } from "@/validations/forest-config.schema";
...
type ImportRow = {
  ...
  name: string;
  description?: string;
  betweenRowsM?: number | null;
  betweenTreesM?: number | null;
  treeDensityPerHa?: number | null;
  isActive?: boolean;
  createdAt?: string;
  updatedAt?: string;
...
export async function POST(req: NextRequest) {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const payload = {
      ...
      betweenRowsM: row.betweenRowsM ?? null,
      betweenTreesM: row.betweenTreesM ?? null,
      treeDensityPerHa: row.treeDensityPerHa ?? null,
      isActive: row.isActive ?? true,
    };

    const parsed = spacingCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.spacing.findFirst({
            where: {
              id: row.id,
      ...
      if (existing) {
        await prisma.spacing.update({
          where: { id: existing.id },
          data: {
            code: parsed.data.code,
            name: parsed.data.name,
            description: parsed.data.description,
            betweenRowsM: parsed.data.betweenRowsM,
            betweenTreesM: parsed.data.betweenTreesM,
            treeDensityPerHa: parsed.data.treeDensityPerHa,
