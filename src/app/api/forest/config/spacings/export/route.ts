...
export async function GET(req: NextRequest) {
  ...
  const spacings = await prisma.spacing.findMany({
    where,
    include: {
      organization: {
  ...
  const rows = spacings.map((item) => ({
    id: item.id,
    code: item.code,
    name: item.name,
    description: item.description ?? "",
    betweenRowsM: decimalToString(item.betweenRowsM),
    betweenTreesM: decimalToString(item.betweenTreesM),
    treeDensityPerHa: decimalToString(item.treeDensityPerHa),
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="spacings_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheetRows = sorted.map((row) => ({
    ...
    "Entre filas (m)": row.betweenRowsM,
    "Entre plantas (m)": row.betweenTreesM,
    "Densidad/ha": row.treeDensityPerHa,
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "Spacings");
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="spacings_${filenameDate}.xlsx"`,
    },
  });
}
