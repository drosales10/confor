...
export async function GET(req: NextRequest) {
  ...
  const [total, items] = await Promise.all([
    prisma.spacing.count({ where }),
    prisma.spacing.findMany({
      where,
      skip: (page - 1) * limit,
      take: limit,
      orderBy: [{ name: "asc" }],
...
export async function POST(req: NextRequest) {
  ...
  const parsed = spacingCreateSchema.safeParse(body);
  if (!parsed.success) return fail("Datos inválidos", 400, parsed.error.flatten());

  try {
    const created = await prisma.spacing.create({
      data: {
        code: parsed.data.code,
        name: parsed.data.name,
        description: parsed.data.description ?? null,
        betweenRowsM: parsed.data.betweenRowsM ?? null,
        betweenTreesM: parsed.data.betweenTreesM ?? null,
        treeDensityPerHa: parsed.data.treeDensityPerHa ?? null,
    ...
    return mapPrismaError(error, "No fue posible crear el espaciamiento");
  }
}
...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json();
  const parsed = spacingUpdateSchema.safeParse(body);
  if (!parsed.success) return fail("Datos inválidos", 400, parsed.error.flatten());

  try {
    const updated = await prisma.spacing.update({
      where: { id: parsed.data.id },
      data: {
        code: parsed.data.code,
        name: parsed.data.name,
        description: parsed.data.description ?? undefined,
        betweenRowsM: parsed.data.betweenRowsM ?? undefined,
        betweenTreesM: parsed.data.betweenTreesM ?? undefined,
        treeDensityPerHa: parsed.data.treeDensityPerHa ?? undefined,
    ...
    return mapPrismaError(error, "No fue posible actualizar el espaciamiento");
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await prisma.spacing.delete({ where: { id: parsed.data.id } });
    ...
    return mapPrismaError(error, "No fue posible eliminar el espaciamiento");
  }
}
