...
function sortRows(rows: ExportRow[], sortBy: SortKey, sortOrder: "asc" | "desc") {
  ...
  return [...rows].sort((left, right) => {
    ...
    if (sortBy === "isProductive") {
      return ((left.isProductive ? 1 : 0) - (right.isProductive ? 1 : 0)) * direction;
    }

    const leftValue = sortBy === "name" ? left.name : sortBy === "category" ? left.category : left.code;
    const rightValue = sortBy === "name" ? right.name : sortBy === "category" ? right.category : right.code;
...
export async function GET(req: NextRequest) {
  ...
  const landUseTypes = await prisma.landUseType.findMany({
    where,
    include: {
      organization: {
  ...
  const rows = landUseTypes.map((item) => ({
    id: item.id,
    code: item.code,
    name: item.name,
    category: (item as { category?: string }).category ?? item.name,
  ...
  const headers = ["id", "code", "name", "category", "isProductive", "isActive", "createdAt", "updatedAt", "organizationId", "organizationName"];
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="land_use_types_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheetRows = sorted.map((row) => ({
    ...
    Productivo: row.isProductive ? "Sí" : "No",
  ...
  const worksheet = XLSX.utils.json_to_sheet(worksheetRows, {
    header: ["ID", "Código", "Nombre", "Categoría", "Productivo", "Activo", "Creado en", "Actualizado en", "Organización ID", "Organización"],
  });

  XLSX.utils.book_append_sheet(workbook, worksheet, "LandUseTypes");
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="land_use_types_${filenameDate}.xlsx"`,
    },
  });
}
