...
import {
  deleteByIdSchema,
  imaClassCreateSchema,
  imaClassUpdateSchema,
...
export async function POST(req: NextRequest) {
  ...
  const parsed = imaClassCreateSchema.safeParse(body);
  ...
  try {
    ...
    return mapPrismaError(error, "No fue posible crear la clase IMA");
  }
}
...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json();
  const parsed = imaClassUpdateSchema.safeParse(body);
  if (!parsed.success) return fail("Datos inválidos", 400, parsed.error.flatten());

  try {
    const updated = await prisma.imaClass.update({
    ...
    return mapPrismaError(error, "No fue posible actualizar la clase IMA");
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await prisma.imaClass.delete({ where: { id: parsed.data.id } });
    ...
    return mapPrismaError(error, "No fue posible eliminar la clase IMA");
  }
}
