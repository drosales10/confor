...
const sortKeys = ["code", "name", "recommendedHarvestType", "isActive", "createdAt", "updatedAt"] as const;
...
type ExportRow = {
  ...
  minLengthM: string;
  maxLengthM: string;
  minSmallEndDiameterCm: string;
  maxSmallEndDiameterCm: string;
  recommendedHarvestType: string;
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
...
function sortRows(rows: ExportRow[], sortBy: SortKey, sortOrder: "asc" | "desc") {
  ...
  return [...rows].sort((left, right) => {
    ...
    if (sortBy === "recommendedHarvestType") {
      return String(left.recommendedHarvestType).localeCompare(String(right.recommendedHarvestType), "es", {
        sensitivity: "base",
      }) * direction;
    }
...
export async function GET(req: NextRequest) {
  ...
  const rows = productTypes.map((item) => ({
    ...
    code: item.code,
    name: item.name,
    minLengthM: decimalToString(item.minLengthM),
    maxLengthM: decimalToString(item.maxLengthM),
    minSmallEndDiameterCm: decimalToString(item.minSmallEndDiameterCm),
    maxSmallEndDiameterCm: decimalToString(item.maxSmallEndDiameterCm),
    recommendedHarvestType: item.recommendedHarvestType,
  ...
  if (format === "csv") {
    const csv = [
      ...
      ...sorted.map((row) =>
        [
          ...
          row.minLengthM,
          row.maxLengthM,
          row.minSmallEndDiameterCm,
          row.maxSmallEndDiameterCm,
          row.recommendedHarvestType,
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="product_types_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheetRows = sorted.map((row) => ({
    ...
    Código: row.code,
    Nombre: row.name,
    "Longitud mín (m)": row.minLengthM,
    "Longitud máx (m)": row.maxLengthM,
    "Diámetro mín (cm)": row.minSmallEndDiameterCm,
    "Diámetro máx (cm)": row.maxSmallEndDiameterCm,
    "Tipo de corte": row.recommendedHarvestType,
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "ProductTypes");
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="product_types_${filenameDate}.xlsx"`,
    },
  });
}
