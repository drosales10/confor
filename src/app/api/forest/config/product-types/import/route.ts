...
import { productTypeCreateSchema } from "@/validations/forest-config.schema";
...
function parseHarvestType(value: string) {
  const normalized = value.trim().toUpperCase();
  if (normalized === "MECANIZADA" || normalized === "MANUAL") {
    return normalized as "MECANIZADA" | "MANUAL";
  }
  return undefined;
}
...
type ImportRow = {
  ...
  code: string;
  name: string;
  minLengthM?: number | null;
  maxLengthM?: number | null;
  minSmallEndDiameterCm?: number | null;
  maxSmallEndDiameterCm?: number | null;
  recommendedHarvestType?: "MECANIZADA" | "MANUAL";
  isActive?: boolean;
  createdAt?: string;
  updatedAt?: string;
...
export async function POST(req: NextRequest) {
  ...
  if (ext === "csv") {
    ...
    const required = ["code", "name", "recommendedharvesttype"];
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const payload = {
      code: row.code,
      name: row.name,
      minLengthM: row.minLengthM ?? null,
      maxLengthM: row.maxLengthM ?? null,
      minSmallEndDiameterCm: row.minSmallEndDiameterCm ?? null,
      maxSmallEndDiameterCm: row.maxSmallEndDiameterCm ?? null,
      recommendedHarvestType: row.recommendedHarvestType,
      isActive: row.isActive ?? true,
    };

    const parsed = productTypeCreateSchema.safeParse(payload);
    if (!parsed.success) {
      ...
      if (existing) {
        await prisma.productType.update({
          ...
          data: {
            ...
            minLengthM: parsed.data.minLengthM,
            maxLengthM: parsed.data.maxLengthM,
            minSmallEndDiameterCm: parsed.data.minSmallEndDiameterCm,
            maxSmallEndDiameterCm: parsed.data.maxSmallEndDiameterCm,
