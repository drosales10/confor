...
import { continentCreateSchema } from "@/validations/forest-config.schema";
...
export async function POST(req: NextRequest) {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const parsed = continentCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.continent.findUnique({ where: { id: row.id } })
        : await prisma.continent.findUnique({ where: { code: parsed.data.code } });
