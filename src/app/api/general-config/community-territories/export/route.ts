...
function sortRows(rows: ExportRow[], sortBy: SortKey, sortOrder: "asc" | "desc") {
  ...
  return [...rows].sort((left, right) => {
    ...
    const mapValue = (row: ExportRow) => {
      if (sortBy === "cityName") return row.cityName;
...
export async function GET(req: NextRequest) {
  ...
  const where = search?.trim()
    ? {
        OR: [
          ...
          { city: { code: { contains: search.trim(), mode: "insensitive" as const } } },
          { city: { name: { contains: search.trim(), mode: "insensitive" as const } } },
          { city: { municipality: { code: { contains: search.trim(), mode: "insensitive" as const } } } },
          { city: { municipality: { name: { contains: search.trim(), mode: "insensitive" as const } } } },
        ],
      }
    : {};

  const communities = await prisma.communityTerritory.findMany({
    where,
    include: {
      city: {
        select: {
  ...
  const rows = communities.map((item) => ({
    ...
    municipalityCode: item.city?.municipality?.code ?? "",
    municipalityName: item.city?.municipality?.name ?? "",
    type: item.type,
    code: item.code,
    name: item.name,
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="community_territories_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheetRows = sorted.map((row) => ({
    ID: row.id,
    "Ciudad ID": row.cityId,
    "Ciudad código": row.cityCode,
    "Ciudad nombre": row.cityName,
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "CommunityTerritories");
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="community_territories_${filenameDate}.xlsx"`,
    },
  });
}
