...
import { communityTerritoryCreateSchema } from "@/validations/forest-config.schema";
...
export async function POST(req: NextRequest) {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    const parsed = communityTerritoryCreateSchema.safeParse(payload);
    if (!parsed.success) {
    ...
    try {
      const existing = row.id
        ? await prisma.communityTerritory.findUnique({ where: { id: row.id } })
        : await prisma.communityTerritory.findFirst({
            where: {
      ...
      if (existing) {
        await prisma.communityTerritory.update({
      ...
      await prisma.communityTerritory.create({
        data: {
          cityId: parsed.data.cityId,
          code: parsed.data.code,
          name: parsed.data.name,
