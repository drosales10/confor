...
function sortRows(rows: ExportRow[], sortBy: SortKey, sortOrder: "asc" | "desc") {
  ...
  return [...rows].sort((left, right) => {
    ...
    const mapValue = (row: ExportRow) => {
      if (sortBy === "continentName") return row.continentName;
...
export async function GET(req: NextRequest) {
  ...
  const where = search?.trim()
    ? {
        OR: [
          ...
          { continent: { name: { contains: search.trim(), mode: "insensitive" as const } } },
          { continent: { code: { contains: search.trim(), mode: "insensitive" as const } } },
        ],
      }
    : {};
  ...
  const rows = countries.map((item) => ({
    ...
    continentCode: item.continent?.code ?? "",
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="countries_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheetRows = sorted.map((row) => ({
    ID: row.id,
    "Continente ID": row.continentId,
    "Continente código": row.continentCode,
    "Continente nombre": row.continentName,
    Código: row.code,
    Nombre: row.name,
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="countries_${filenameDate}.xlsx"`,
    },
  });
}
