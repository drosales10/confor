...
function sortRows(rows: ExportRow[], sortBy: SortKey, sortOrder: "asc" | "desc") {
  ...
  return [...rows].sort((left, right) => {
    ...
    const mapValue = (row: ExportRow) => {
      if (sortBy === "municipalityName") return row.municipalityName;
      if (sortBy === "stateName") return row.stateName;
      if (sortBy === "countryName") return row.countryName;
      if (sortBy === "name") return row.name;
      return row.code;
    };

    return (
      String(mapValue(left) ?? "").localeCompare(String(mapValue(right) ?? ""), "es", {
        sensitivity: "base",
      }) * direction
    );
  });
...
export async function GET(req: NextRequest) {
  ...
  const where = search?.trim()
    ? {
        OR: [
          ...
          { municipality: { code: { contains: search.trim(), mode: "insensitive" as const } } },
          { municipality: { name: { contains: search.trim(), mode: "insensitive" as const } } },
          { municipality: { state: { code: { contains: search.trim(), mode: "insensitive" as const } } } },
          { municipality: { state: { name: { contains: search.trim(), mode: "insensitive" as const } } } },
          { municipality: { state: { country: { code: { contains: search.trim(), mode: "insensitive" as const } } } } },
          { municipality: { state: { country: { name: { contains: search.trim(), mode: "insensitive" as const } } } } },
        ],
      }
    : {};
  ...
  const rows = cities.map((item) => ({
    id: item.id,
    municipalityId: item.municipalityId,
    municipalityCode: item.municipality?.code ?? "",
    municipalityName: item.municipality?.name ?? "",
    stateCode: item.municipality?.state?.code ?? "",
    stateName: item.municipality?.state?.name ?? "",
    countryCode: item.municipality?.state?.country?.code ?? "",
    countryName: item.municipality?.state?.country?.name ?? "",
    code: item.code,
    name: item.name,
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="cities_${filenameDate}.csv"`,
      },
    });
  }
  ...
  const worksheetRows = sorted.map((row) => ({
    ID: row.id,
    "Municipio ID": row.municipalityId,
    "Municipio código": row.municipalityCode,
    "Municipio nombre": row.municipalityName,
    "Estado código": row.stateCode,
    "Estado nombre": row.stateName,
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="cities_${filenameDate}.xlsx"`,
    },
  });
}
