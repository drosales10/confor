...
export async function GET(req: NextRequest) {
  ...
  const where = search?.trim()
    ? {
        OR: [
          ...
          { state: { code: { contains: search.trim(), mode: "insensitive" as const } } },
          { state: { name: { contains: search.trim(), mode: "insensitive" as const } } },
          { state: { country: { code: { contains: search.trim(), mode: "insensitive" as const } } } },
          { state: { country: { name: { contains: search.trim(), mode: "insensitive" as const } } } },
        ],
      }
    : {};
  ...
  const rows = municipalities.map((item) => ({
    id: item.id,
    stateId: item.stateId,
    stateCode: item.state?.code ?? "",
    stateName: item.state?.name ?? "",
    countryCode: item.state?.country?.code ?? "",
  ...
  if (format === "csv") {
    ...
    return new NextResponse(csv, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="municipality_districts_${filenameDate}.csv"`,
      },
    });
  }
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, "MunicipalityDistricts");
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="municipality_districts_${filenameDate}.xlsx"`,
    },
  });
}
