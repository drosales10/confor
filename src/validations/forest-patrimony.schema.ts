...
const level2Schema = z.object({
  ...
  type: z.enum(["FINCA", "PREDIO", "HATO", "FUNDO", "HACIENDA", "ABRAE"]),
  legalDocumentDate: z.coerce.date().optional(),
  legalStatus: z.enum(["ADQUISICION", "ARRIENDO", "USUFRUCTO", "COMODATO", "DECRETO"]).optional(),
...
const level3Schema = z.object({
  ...
  code: z.string().min(1).max(80),
  name: z.string().min(2).max(255),
  type: z.enum(["COMPARTIMIENTO", "BLOCK", "SECCION", "LOTE", "ZONA", "BLOQUE", "ZONIFICACION"]),
  totalAreaHa: decimalField,
  centroidLatitude: z.coerce.number().min(-90).max(90).optional(),
  centroidLongitude: z.coerce.number().min(-180).max(180).optional(),
  lastInfoDate: z.coerce.date().optional(),
...
const level4Schema = z.object({
  ...
  code: z.string().min(1).max(80),
  name: z.string().min(2).max(255),
  type: z.enum(["RODAL", "PARCELA", "ENUMERATION", "UNIDAD_DE_MANEJO", "CONUCO", "OTRO_USO"]),
  fscCertificateStatus: z.enum(["SI", "NO"]),
  currentLandUseName: z.string().max(255).optional(),
  previousLandUseName: z.string().max(255).optional(),
  totalAreaHa: decimalField,
